...
exports.storeAccounts = async (data) => {
  ...
  const userId = await Users.findOne({ email: userInfo.email })
    .select("_id email")
    .exec();
  console.log("userId: ", userId);
  const key = `${companyInfo.CompanyInfo.Id}-${userId.email}`;
  let company = await Companies.findOne({ Kibi_CompanyId: key })
    .select("_id")
    .exec();
  ...
  accounts.Rows.Row.map(async (account) => {
    ...
    let account01 = await Accounts.findOne({
    ...
    if (!account01) {
      ...
      account01 = await Accounts.findOneAndUpdate(
        {
          Kibi_AccountId: accountKey,
        },
        {
...
exports.getAccounts = async (req, res) => {
  ...
  const accounts = await Accounts.find({ Kibi_CompanyId: company_Id._id })
...
exports.changeAccountStatus = async (req, res) => {
  ...
  const allAvailable = await Accounts.find({
    Kibi_AvailableForSelection: true,
  }).exec();
  const allBusy = await Accounts.find({
    Kibi_AvailableForSelection: false,
  }).exec();
