...
const Index = () => {
	...
	const { data: accounts = [], isLoading: accountsLoading } = useGetAccountsQuery(companyId);
	const { data: selectAllValu, isLoading: selectAllValueLoading } = useGetSelectAllAccountsValueQuery();
	const [updateAccountStatus, result] = useUpdateAccountStatusMutation()
	const [changeAllAccountsAvailabilityStatus, { result: changeAllAccountsAvailabilityStatusResponse }] = useChangeAllAccountsAvailabilityStatusMutation()
	const [quickbookaccounts, setQuickbookAccounts] = useState();
	const subledgerAccounts = accounts.filter(row => row.DetailType === 'Prepaid Expenses' ? true : false)
	...
	const handleStatusChange = async (event, state, setState, _id) => {
		...
		const switchEnable = newRows.filter((row) => row.Kibi_AvailableForSelection == false)
		...
		await updateAccountStatus({ value: !state, id: _id }).then().catch((error) => {
			setState(state);
		})
	};
	const handleSelectAllChange = async (event, state, setState) => {
		...
		setState(!state);
		await changeAllAccountsAvailabilityStatus({ value: !state }).then().catch((error) => {
			setState(state);
		})
	}
